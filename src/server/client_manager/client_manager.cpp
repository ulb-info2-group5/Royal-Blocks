...
void ClientManager::sendUpdatedRankingToClients() const {
    std::vector<std::pair<std::string, size_t>> ranking = accountService_.getRanking();
...
void ClientManager::handlePacket(const std::string &packet,const UserID &userID) {
...
void ClientManager::handlePacketMenu(const std::string &packet, const UserID &userID) {
    ...
    case bindings::BindingType::JoinGame:
        ...
        connectedClients_[userID]->setGameMode(bindings::JoinGame::from_json(jPack).gameMode);
    ...
    case bindings::BindingType::CreateGame:
        connectedClients_[userID]->setUserState(bindings::State::Matchmaking);
        connectedClients_[userID]->setGameMode(bindings::CreateGame::from_json(jPack).gameMode);
        updateThisUserWithAllhisFriends(userID);
    ...
    case bindings::BindingType::FriendRequest:
        socialService_.handleFriendRequest(userID, bindings::FriendRequest::from_json(jPack),accountService_.getAccountManager());
        updateMenu(userID);
        updateMenu(accountService_.getUserID(jPack.at("data").at("targetName").get<std::string>()));
        break;
    case bindings::BindingType::Message:
        socialService_.handleMessages(userID, bindings::Message::from_json(jPack));
        updateMenu(userID);
        updateMenu(jPack.at("data").at("recipientId").get<UserID>());
        break;
    case bindings::BindingType::HandleFriendRequest:
        
        socialService_.handleHandleFriendRequest(userID, bindings::HandleFriendRequest::from_json(jPack));
        updateMenu(userID);
        updateMenu(jPack.at("data").at("user").get<UserID>());
        break;
    case bindings::BindingType::RemoveFriend:
        socialService_.handleRemoveFriend(userID, bindings::RemoveFriend::from_json(jPack));
        updateMenu(userID);
        updateMenu(jPack.at("data").at("playerId").get<UserID>());
        break;
    case bindings::BindingType::ChangePassword:
        accountService_.changePassword(userID, bindings::ChangePassword::from_json(jPack));
        break;
    case bindings::BindingType::ChangeUsername:
        connectedClients_[userID]->sendPackage(accountService_.attemptChangeUsername(userID, bindings::ChangeUsername::from_json(jPack), 
...
void ClientManager::updateMenu(UserID userID){
    if (getUserState(userID) == bindings::State::Menu ){
        ...
        connectedClients_[userID]->sendPackage(socialService_.getConversations(userID, accountService_.getAccountManager()).to_json());
    }
