...
void GameDisplay::playerInfo() {
    playerInfo_.setText(getSelfUsername() + " : " + std::to_string(getSelfScore()));
...
void GameDisplay::holdTetromino() {
    ...
    if (holdTetromino_.isNull()) holdTetromino_ = QImage(width, height, QImage::Format_ARGB32);
...
void GameDisplay::drawBoard(CellSize size) {
    ...
    if (selfBoard_.isNull()) selfBoard_ = QImage(cellSize * width, cellSize * height, QImage::Format_ARGB32);
...
void GameDisplay::tetrominoQueue() {
    ...
    if (tetrominoQueue_.isNull()) tetrominoQueue_ = QImage(width, height, QImage::Format_ARGB32);
    ...
    for (size_t idx = 0; idx < queueSize; ++idx) {
        ...
        QColor color =
            getQColor(colorIdToColor(tetromino.colorId));
...
void GameDisplay::middlePane() {
    ...
    if (isSpectating()) {
        ...
        tetrominoQueueLabel_->setPixmap(QPixmap::fromImage(tetrominoQueue_));
...
void GameDisplay::createOpBoardDisplay(size_t index, CellSize size) {
    ...
    for (uint32_t y = 0; y < height; ++y) {
        for (uint32_t x = 0; x < width; ++x) {
            ...
            QColor color =
                getQColor(opponentsBoardGetColorIdAt(index, x, y)
                                    .transform([](auto id) {
            ...
            for (uint32_t dy = 0; dy < cellSize; ++dy) {
                for (uint32_t dx = 0; dx < cellSize; ++dx) {
                    opBoards_.at(index).setPixelColor(x * cellSize + dx,
                                        (height - 1 - y) * cellSize + dy,
                                        color);
                }
            }
...
void GameDisplay::rightPane() {
    ...
    for (size_t rowIdx = 0; rowIdx < numRows; rowIdx++) {
        ...
        for (size_t j = 0;
                j < numCols && rowIdx * numCols + j < opSingularDisplay_.size();
                ++j) {
                opLayout_.addWidget(
                    opSingularDisplay_.at(rowIdx * numCols + j),
                    rowIdx,
                    j
                );
        }
...
void GameDisplay::setup() {
    QPushButton *quitButton_ = new QPushButton;
    ...
    if (getGameMode() != GameMode::Endless) ( QGridLayout *opLayout_ = new QGridLayout; )
