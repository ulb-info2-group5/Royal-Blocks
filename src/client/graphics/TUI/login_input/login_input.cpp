...
namespace TUI {
    ...
    LoginInput::LoginInput(MainTui &mainTui, Controller &controller,
        ...
        inputPassword_ =
            ftxui::Input(&password_, std::string(STR_ENTER_PASSWORD),
                         PasswordInputOption())
            | ftxui::borderHeavy
            | ftxui::CatchEvent([this](const ftxui::Event &event) {
                  if (event == ftxui::Event::Return) {
        ...
        displayWindow_ = ftxui::Renderer(
            ...
            [&] {
                ...
                if (!instruction_.empty()) {
                    ...
                    elements.push_back(ftxui::text(instruction_)
                                       | ftxui::center);
                    elements.push_back(ftxui::separator());
                }
                ...
                if (!errorMessage_.empty()) {
                    elements.push_back(ftxui::text(errorMessage_)
                                       | ftxui::center
                ...
                return ftxui::vbox(elements) | ftxui::borderHeavy
                       | ftxui::center;
            });
    }
