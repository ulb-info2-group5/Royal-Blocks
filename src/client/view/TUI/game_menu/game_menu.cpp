...
GameMenu::GameMenu(std::shared_ptr<ftxui::ScreenInteractive> screen, Controller *controller) : screen_(screen), controller_(controller), joinType_(JoinType::NONE), typeGame_(TypeGame::NONE) {
    endlessButon_ = ftxui::Button("Endless", [&] { 
        gameDisplay_ = std::make_unique<GameDisplay>(screen_, controller_, controller_->getBoards(), PlayMode::ENDLESS);
        joinType_ = JoinType::ENDLESS;
        screen_->ExitLoopClosure()();
    }, ftxui::ButtonOption::Animated(ftxui::Color::Grey0)) | ftxui::border;

    duelButon_ = ftxui::Button("Duel", [&] { 
        gameDisplay_ = std::make_unique<GameDisplay>(screen_, controller_, controller_->getBoards(), PlayMode::DUEL);
        joinFriendOrRandomScreen();
        screen_->ExitLoopClosure()();
    }, ftxui::ButtonOption::Animated(ftxui::Color::Grey0)) | ftxui::border;

    classicButon_ = ftxui::Button("Classic", [&] { 
        gameDisplay_ = std::make_unique<GameDisplay>(screen_, controller_, controller_->getBoards(), PlayMode::CLASSIC);
        joinFriendOrRandomScreen();
        screen_->ExitLoopClosure()();
    }, ftxui::ButtonOption::Animated(ftxui::Color::Grey0)) | ftxui::border;
    
    royalButon_ = ftxui::Button("Royal", [&] { 
        gameDisplay_ = std::make_unique<GameDisplay>(screen_, controller_, controller_->getBoards(), PlayMode::ROYAL);
        joinFriendOrRandomScreen();
        screen_->ExitLoopClosure()();
    }, ftxui::ButtonOption::Animated(ftxui::Color::Grey0)) | ftxui::border;
...
void GameMenu::joinFriendOrRandomScreen() {
    ftxui::Component joinFriendButton = ftxui::Button("Join a friend", [&] {
        joinType_ = JoinType::FRIEND;
        screen_->ExitLoopClosure()();
    }, ftxui::ButtonOption::Animated(ftxui::Color::Grey0)) | ftxui::border;
    ftxui::Component joinRandomButton = ftxui::Button("Join a random game", [&] {
    ...
    ftxui::Component renderer = ftxui::Renderer(container, [&] {
        return ftxui::vbox({
            ftxui::text("Join a friend or a random game") | ftxui::center | ftxui::bold,
            ftxui::separator(),
...
void GameMenu::handleChoice() {
    switch (joinType_) {
        ...
        case JoinType::ENDLESS:
            gameDisplay_->render(); // Endless mode is directly started without waiting for a friend or a random game
            break;
        ...
        default:
            throw std::invalid_argument("Invalid state in GameMenu::handleChoice()");
            break;
    }
}
...
void GameMenu::joinFriendScreen() {
    ...
    ftxui::Component friendsContainer = ftxui::Container::Vertical(friendButtons);
    ...
    ftxui::Component renderer = ftxui::Renderer(friendsContainer, [&] {
        return ftxui::vbox({
            ftxui::text("Select a friend to join") | ftxui::center | ftxui::bold,
            ftxui::separator(),
...
void GameMenu::joinRandomScreen() {
    ftxui::Component renderer = ftxui::Renderer(ftxui::Container::Vertical({}), [&] {
        return ftxui::vbox({
            ftxui::text("Searching for a random game") | ftxui::center | ftxui::bold,
            ftxui::separator(),
...
ftxui::Component GameMenu::makeFriendButton(const std::string &friendName) {
    return ftxui::Button(friendName, [&] {
...
void GameMenu::waitingFriendScreen() {
    ...
    ftxui::Component renderer = ftxui::Renderer(ftxui::Container::Vertical({}), [&] {
        return ftxui::vbox({
            ftxui::text("Waiting for the friend to start the game") | ftxui::center | ftxui::bold,
            ftxui::separator(),
            ftxui::text("Please wait ...") | ftxui::center,
        }) | ftxui::border | ftxui::center | ftxui::bgcolor(ftxui::Color::Black);
    });
...
void GameMenu::render(const TypeGame &typeGame) {
    ...
    switch (typeGame_) {
        ...
        default:
            throw std::invalid_argument("Invalid type of game in GameMenu::render()");
            break;
    }
}